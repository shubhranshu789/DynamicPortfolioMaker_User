...
router.post("/signup" , (req,res)=> {
    ...
    ADMIN.findOne({$or : [{email : email} , {userName: userName} , {accesspoint : accesspoint}]}).then((savedUser) => {
        if(savedUser){
            if(savedUser.email === email){
                return res.status(422).json({error : "User already exist with that email"})
            }
            if(savedUser.userName === userName) {
                return res.status(422).json({error : "User already exist with that userName"})
            }
            if(savedUser.accesspoint === accesspoint){
                return res.status(422).json({error : "Access point already exist "})
            }
        }
